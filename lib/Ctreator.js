...
class Ctreator {
    ...
    async fetchRepo(){
        ...
        if(!repos)return
        repos = repos.map(item => item.name).filter(name => name != 'create-react')
    ...
    async downLoad(repo,tag){
        ...
        await wrapLoading(this.downloadGitRepo,'waiting download templete',requestUrl,path.resolve(process.cwd(),`${this.name}`))
